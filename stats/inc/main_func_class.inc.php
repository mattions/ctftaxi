...
function php_stats_getbrowser($arg){
    ...
    for($i=0,$tot=count($bw_def);$i<$tot;++$i)
        ...
        if(strpos($arg,$id_bw)===FALSE) continue; //non è stato trovato la stringa del nome del browser
        ...
        if($tmp===FALSE) continue; //non è stata trovata la stringa che precede la versione
        $startpos=$tmp+strlen($pre_ver);
                $res=preg_match('/[0-9\.]+/', substr($arg,$startpos), $matches);
        if($res<1) continue; // non è stato trovato il numero di versione
        $version=$matches[0];
...
function php_stats_getos($arg){
  ...
  for($i=0,$tot=count($os_def);$i<$tot;++$i)
    ...
    if(strpos($arg,$id_os)===FALSE) continue;//non è stata trovata la stringa del sistema operativo
...
function php_stats_getengine($reffer){
   ...
   if(strpos($reffer,'?')===FALSE){
      ...
      else return FALSE; //se non c'è querystring e non è url rewriting non è un motore;
   }
   ...
   for($i=0,$tot=count($search_engines_def);$i<$tot;++$i){
      ...
      if($forcedDomain && ($tmpdomain=='com' || $tmpdomain=='net' || $tmpdomain=='org')) $domain=$forcedDomain;//se è presente il dominio forzato lo uso al posto di com,org,net
