...
// ACCESSI TOTALI
$result=sql_query("SELECT hits,visits,no_count_hits,no_count_visits FROM $option[prefix]_counters");
...
if($modulo[6]):
  ...
  list($hits_oggi,$visite_oggi)=@mysql_fetch_row($result);
  ...
  list($hits_ieri,$visite_ieri)=@mysql_fetch_row($result);
  ...
  list($hits_questo_mese,$visite_questo_mese)=@mysql_fetch_row($result);
  ...
  list($hits_scorso_mese,$visite_scorso_mese)=@mysql_fetch_row($result);
  ...
  list($hits_this_year,$visite_this_year)=@mysql_fetch_row($result);
...
if($modulo[3]):
  ...
  $result_ol=sql_query("SELECT data FROM $option[prefix]_cache WHERE data>$date AND os NOT REGEXP 'Spider|Grabber'");
...
if(@mysql_num_rows($result)>0) list($most_used_engine,$most_used_engine_volte)=@mysql_fetch_row($result);
...
if($online>0) {
  ...
  while($row=@mysql_fetch_array($result,MYSQL_ASSOC))
    ...
    if($option['page_title']==1)
      ...
      if($option['ext_whois']=='') $return.="<a href=\"javascript:whois('whois.php?IP=$row[user_id]');\">$row[user_id]</a>";
      else $return.="<a href=\"".str_replace('%IP%',$row['user_id'],$option['ext_whois'])."\" target=\"_BLANK\">$row[user_id]</a>";
      $return.=
      "</span></td>".
      ...
      "\n\t<td bgcolor=$style[table_bgcolor] width=\"11\"><a href=\"javascript:popup('tracking.php?what=online&ip=$row[user_id]');\"><img src=\"templates/$option[template]/images/icon_tracking.gif\" border=0 title=\"".$string['main_track_alt']."\"></a></td>".
      "\n\t</tr>";
    }
  $return.=
  "\n\t<tr>".
...
// Utente Medio //
//////////////////
$result=sql_query("SELECT os,SUM(visits) AS dummy FROM $option[prefix]_systems GROUP BY os ORDER BY 'dummy' DESC LIMIT 2");
$au_os='-';
$icon_os='';
if(@mysql_num_rows($result)>0)
  {
  while(list($tmp,$dummy)=@mysql_fetch_row($result))
    {
    if($au_os!=='?')  //scarto gli unknown
      {
      $au_os=$tmp;
...
$result=sql_query("SELECT bw,SUM(visits) AS dummy FROM $option[prefix]_systems GROUP BY bw ORDER BY 'dummy' DESC LIMIT 2");
$au_bw='-';
$icon_bw='';
if(@mysql_num_rows($result)>0)
  {
...
$result=sql_query("SELECT reso,SUM(visits) AS dummy FROM $option[prefix]_systems GROUP BY reso ORDER BY 'dummy' DESC LIMIT 2");
$au_resolution='-';
if(@mysql_num_rows($result)>0)
  {
  while(list($tmp,$dummy)=@mysql_fetch_row($result))
...
$result=sql_query("SELECT colo,SUM(visits) AS dummy FROM $option[prefix]_systems GROUP BY colo ORDER BY 'dummy' DESC LIMIT 2");
$au_colors='-';
if(@mysql_num_rows($result)>0)
  {
  while(list($tmp,$dummy)=@mysql_fetch_row($result))
...
$result=sql_query("SELECT tld,area,visits AS dummy FROM $option[prefix]_domains ORDER BY 'dummy' DESC LIMIT 2");
$au_country='-';
while(list($tmp,$au_area,$dummy)=@mysql_fetch_row($result))
  {
  if($tmp!=='unknown' && $tmp!=='lan' && $dummy>0) //scarto gli unknown e considero il caso della tabella vuota
    {
...
while(list($tmp,$dummy)=@mysql_fetch_row($result))
  {
  if($tmp!=='unknown' && $dummy>0) //scarto gli unknown
    {
