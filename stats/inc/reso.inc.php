...
if($mode==0) $phpstats_title=str_replace(Array('%MESE%','%ANNO%'),Array(formatmount($sel_mese),$sel_anno),$string['reso_colo_title_2']);
else $phpstats_title=$string['reso_colo_title'];

$return='';
if(strlen("$sel_mese")<2) $sel_mese='0'.$sel_mese;
$clause=($mode==0 ? "WHERE mese='$sel_anno-$sel_mese' AND reso<>''" : "WHERE reso<>''");
...
if($num_totale>0)
  ...
  if($mode==0) $tmp=str_replace(Array('%MESE%','%ANNO%'),Array(formatmount($sel_mese),$sel_anno),$string['reso_title_2']);
  else $tmp=$string['reso_title'];
  $return.="<span class=\"pagetitle\">$tmp</span>";
  //
  $count=0;
  $result=sql_query("SELECT reso,SUM(hits) AS sumhits,SUM(visits) as sumvisits FROM $option[prefix]_systems $clause GROUP BY reso ORDER BY 'sumhits' DESC");
  $return.=
  "<br><br><table border=\"0\" width=\"90%\" $style[table_header] align=\"center\">".
  ...
  if($mode==0) $tmp=str_replace(Array('%MESE%','%ANNO%'),Array(formatmount($sel_mese),$sel_anno),$string['colo_title_2']);
  else $tmp=$string['colo_title'];
  $return.="<span class=\"pagetitle\">$tmp</span>";
  //
  $clause=($mode==0 ? "WHERE mese='$sel_anno-$sel_mese' AND colo<>''" : "WHERE colo<>''");
  
  $result=sql_query("SELECT colo,SUM(hits) AS sumhits,SUM(visits) AS sumvisits FROM $option[prefix]_systems $clause GROUP BY colo ORDER BY 'sumhits' DESC");
  $return.=
  "<br><br><table border=\"0\" width=\"90%\" $style[table_header] align=\"center\">".
  "<tr><td bgcolor=$style[table_title_bgcolor] nowrap><span class=\"tabletitle\"><center>$string[colo_colo]</center></span></td><td bgcolor=$style[table_title_bgcolor] nowrap><span class=\"tabletitle\"><center>$string[colo_hits]</center></span></td><td bgcolor=$style[table_title_bgcolor] nowrap><span class=\"tabletitle\"><center></center></span></td></tr>";
  while($row=@mysql_fetch_array($result,MYSQL_ASSOC))
    {
    if($row['colo']!=='?') $row['colo'].=' bit';
    $return.=
    "<tr onmouseover=\"setPointer(this, '$style[table_hitlight]', '$style[table_bgcolor]')\" onmouseout=\"setPointer(this, '$style[table_bgcolor]', '$style[table_bgcolor]')\">".
  ...
  {
  if($mode==1) $return.=info_box($string['information'],$error['reso']);
  else
    {
...
if($modulo[1]==2)
  ...
  if($mode==0)
    ...
    $return.=
    "<form action='./admin.php?action=reso' method='POST' name=form1><span class=\"tabletextA\">$string[calendar_view]</span>".
  ...
  else $return.="<a href=\"admin.php?action=reso&mode=0\"><img src=templates/$option[template]/images/icon_change.gif border=\"0\" align=\"absmiddle\" hspace=\"1\" vspace=\"1\"><span class='testo'>  $string[os_vis_mens]</span></a>";
  $return.='</center>';
  }
return($return);
}
