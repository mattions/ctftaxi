...
if($mode==0){
        if($q==''){
                ...
                $clause_expl="WHERE data='$query_details' AND engine='$engine_details' AND domain='$domain_details' AND mese='$sel_anno-$sel_mese'";
        }
        else{
                $clause="WHERE mese='$sel_anno-$sel_mese' AND data LIKE '%$q%'";
                $clause_expl="WHERE data='$query_details' AND engine='$engine_details' AND domain='$domain_details' AND mese='$sel_anno-$sel_mese' AND data LIKE '%$q%'";
        }
}
else if($mode==2){
        if($q==''){
                $clause="WHERE mese='$sel_anno-$sel_mese'";
                $clause_expl="WHERE data='$query_details' AND mese='$sel_anno-$sel_mese'";
        }
        else{
                $clause="WHERE mese='$sel_anno-$sel_mese' AND data LIKE '%$q%'";
                $clause_expl="WHERE data='$query_details' AND mese='$sel_anno-$sel_mese' AND data LIKE '%$q%'";
        }
}
else{
        if($q==''){
                $clause='';
                $clause_expl="WHERE data='$query_details'";
        }
        else{
                $clause="WHERE data LIKE '%$q%'";
                $clause_expl="WHERE data='$query_details' AND data LIKE '%$q%'";
        }
}
